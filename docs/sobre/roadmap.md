# Roadmap da Especialização

Visão geral do planejamento, marcos e evolução prevista para o programa de especialização em Programação Agêntica.

## Documento Principal

- [Roadmap Completo](../00-Governanca/roadmap.md) — calendário de entregas, marcos e dependências entre módulos

## O que você encontrará

- Cronograma das turmas e ciclos de oferta
- Marcos de desenvolvimento do programa
- Dependências entre disciplinas

## Arquivos para consultar

- `00-Governanca/roadmap.md`
